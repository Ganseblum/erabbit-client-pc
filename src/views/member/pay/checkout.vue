...
export default {
  ...
  setup() {
    ...
    findCheckoutInfo().then((data) => {
      ...
      const aa = checkoutInfo.value.userAddresses.find(
        (item) => item.isDefault === 0
      )
      if (aa) {
        reqParams.addressId = aa.id
      } else {
        reqParams.addressId = checkoutInfo.value.userAddresses[0].id
      }
    ...
    // 改变提交商品的的地址
    const changAddressId = (address) => {
      reqParams.addressId = address
    ...
    return { checkoutInfo, submitOrder, reqParams, changAddressId }
  }
}
</script>
