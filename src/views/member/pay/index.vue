<template>
  <div class="xtx-pay-page">
    <div class="container">
      ...
      <!-- 付款信息 -->
      <div class="pay-info" v-if="orderGoods">
        <span class="icon iconfont icon-queren2"></span>
        <div class="tip" v-if="timeText">
          <p>订单提交成功！请尽快完成支付。</p>
          <p v-if="orderGoods.countdown > -1">
            支付还剩 <span>{{ timeText }}</span
            >, 超时后将取消订单
          </p>
          <p v-else>订单已经超时</p>
        </div>
        <div class="amount" v-if="orderGoods.totalMoney">
          <span>应付总额：</span>
          <span>¥{{ orderGoods.totalMoney }}</span>
        </div>
      </div>
      <!-- 付款方式 -->
      <div class="pay-type">
  ...
  <XtxDialog title="正在支付..." v-model:visible="visibleDialog">
    <div class="pay-wait">
      ...
      <div v-if="orderGoods">
        <p>如果支付成功：</p>
        <RouterLink :to="`/member/order?orderId=${orderGoods.id}`"
          >查看订单详情></RouterLink
        >
        <p>如果支付失败：</p>
  ...
  setup() {
    ...
    findOrder(route.query.orderId).then((data) => {
      ...
      if (data.result.countdown > -1) {
        const time = data.result.countdown
        start(time)
      }
    })
    ...
    return { orderGoods, timeText, payUrl, visibleDialog }
  }
}
</script>
